...
public class ItemStorage {
    ...
    public void writeObjectToFile(Item anItem) {
        try {
            ObjectOutputStream output = new ObjectOutputStream(new FileOutputStream(file));
            output.writeObject(anItem);
            output.flush();
            output.close();
    ...
    public void readObjectFromFile() {
        ...
        try {
            if (file.exists()) {
                ...
                Item loadedItem = (Item) input.readObject();
                while (loadedItem != null) {
                    loadedItem = items.put(loadedItem.getUpc(), loadedItem);
                }
                input.close();
            } else {
                ObjectOutputStream output = new ObjectOutputStream(new FileOutputStream(file));
